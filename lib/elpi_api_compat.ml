(** Compatibility shim providing fork-specific Elpi API additions.
    Replaces the Gopiandcode/elpi fork's additions to ContextualConversion,
    BuiltInContextualData, Builtin.PPX, and PPX.Doc. *)

open Elpi.API

(** Base class wrapping raw hypothetical context.
    Provides the same interface as the fork's [ContextualConversion.ctx]. *)
...
(** A context descriptor: everything needed to read back a contextual type
    from the hypothetical context.  The [conv] type uses [unit] for the
    [hyps] parameter because the embed/readback functions ignore the context
    object (they rely on state instead). *)
...
(** Process the hypothetical context: for each hyp that corresponds to a
    context entry (as determined by [ctx.is_entry_for_nominal]), read back
    the entry and register it in the state via [ctx.push].  Returns the
    updated state and accumulated extra goals. *)
  ...
  List.fold_left (fun (state, gls) hyp ->
    ...
    | Some nominal ->
        ...
        let (state, (_, entry), entry_gls) =
          ctx.conv.ContextualConversion.readback
            ~depth:depth_hyp () csts state raw.RawData.hsrc in
...
(** A fork-compatible ctx_readback type.  The first parameter is the
    CONSTRUCTED context class type (not the raw input hyps type).  The
    function receives raw [Data.hyps] and returns the constructed class. *)
...
module BuiltInContextualData = struct
  ...
  let nominal : (RawData.constant, 'ctx, 'csts) ContextualConversion.t =
    ...
    let embed ~depth:_ _ctx _csts s c =
      (s, RawData.mkBound c, []) in
      ...
      | _ ->
          Utils.type_error
            (Format.asprintf "Not a nominal (bound variable): %a"
               (RawPp.term depth) t) in
    { ContextualConversion.ty; pp_doc = (fun _ () -> ()); pp = (fun fmt c -> Format.fprintf fmt "#%d" c);
      embed; readback }
...
module PPX = struct
  ...
  let char : (char, 'ctx, 'csts) ContextualConversion.t =
    ...
    let embed ~depth _ctx _csts s c =
      BuiltInData.string.embed ~depth s (String.make 1 c) in
    let readback ~depth _ctx _csts s t =
      let (s, str, gls) = BuiltInData.string.readback ~depth s t in
      if String.length str = 1 then (s, str.[0], gls)
      else Utils.type_error "expected a single-character string for char" in
  ...
  let triple_decl (a : ('a, 'h, 'c) ContextualConversion.t)
    ...
    Decl {
      ...
      constructors = [
        K ("trpl", "trpl", CA (a, CA (b, CA (cc, N))),
          B (fun x y z -> (x, y, z)),
          M (fun ~ok ~ko:_ -> function (x, y, z) -> ok x y z));
      ]
    }
  ...
  let quadruple_decl (a : ('a, 'h, 'c) ContextualConversion.t)
    ...
    Decl {
      ...
      doc = "Quadruples";
      pp = (fun fmt (x, y, z, w) ->
        Format.fprintf fmt "quadr(%a,%a,%a,%a)"
          a.ContextualConversion.pp x b.ContextualConversion.pp y
          cc.ContextualConversion.pp z d.ContextualConversion.pp w);
      constructors = [
        K ("quadr", "quadr", CA (a, CA (b, CA (cc, CA (d, N)))),
          B (fun x y z w -> (x, y, z, w)),
          M (fun ~ok ~ko:_ -> function (x, y, z, w) -> ok x y z w));
      ]
    }
  ...
  let quintuple_decl (a : ('a, 'h, 'c) ContextualConversion.t)
    ...
    Decl {
      ...
      doc = "Quintuples";
      pp = (fun fmt (x, y, z, w, v) ->
        Format.fprintf fmt "quint(%a,%a,%a,%a,%a)"
          a.ContextualConversion.pp x b.ContextualConversion.pp y
          cc.ContextualConversion.pp z d.ContextualConversion.pp w
          e.ContextualConversion.pp v);
      constructors = [
        K ("quint", "quint", CA (a, CA (b, CA (cc, CA (d, CA (e, N))))),
...
module Doc = struct
  ...
  let rec show_ty_ast ?(prec = Arrow) =
    ...
    | TyApp (s, a, []) ->
        ...
        (match prec with AppArg -> "(" ^ inner ^ ")" | Arrow -> inner)
    | TyApp (s, a, rest) ->
        let args = String.concat " " (List.map (show_ty_ast ~prec:AppArg) (a :: rest)) in
