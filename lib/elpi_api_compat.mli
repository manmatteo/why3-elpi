...
(** Base class wrapping raw hypothetical context.
    Drop-in for the fork's [Elpi.API.ContextualConversion.ctx]. *)
class ctx : Elpi.API.Data.hyps ->
  object
...
val pp_ctx_field :
  (Format.formatter -> 'a -> unit) ->
  Format.formatter -> 'a ctx_field -> unit
...
(** Context descriptor – replaces the fork's [Elpi.API.ContextualConversion.context]. *)
type ('a, 'k, 'csts) context = {
  is_entry_for_nominal : Elpi.API.Data.hyp -> Elpi.API.RawData.constant option;
  to_key               : depth:int -> 'a -> 'k;
  push                 : depth:int -> Elpi.API.Data.state -> 'k -> 'a ctx_entry -> Elpi.API.Data.state;
  pop                  : depth:int -> Elpi.API.Data.state -> 'k -> Elpi.API.Data.state;
  conv                 : (Elpi.API.RawData.constant * 'a, unit, 'csts) Elpi.API.ContextualConversion.t;
  init                 : Elpi.API.Data.state -> Elpi.API.Data.state;
  get                  : Elpi.API.Data.state -> 'a ctx_field;
}

(** Process hyps into state.  Drop-in for the fork's [Elpi.API.PPX.readback_context]. *)
