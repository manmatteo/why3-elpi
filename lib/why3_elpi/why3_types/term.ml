...
type vsymbol = {
  ...
  Elpi.API.OpaqueData.name = "var"; doc = "Embedding of variable symbols"; pp = pp_why_ident Why3.Pretty.print_vs;
  compare = vs_compare;
  hash = Hashtbl.hash;
  hconsed = false;
  constants = [];
...
type term = {
  ...
  Elpi.API.OpaqueData.name = "term";
  doc = "TODO Term";
  pp = (fun fmt t -> Format.fprintf fmt "_term");
  compare = (fun _ _ -> 0);
  hash = (fun _ -> 0);
  hconsed = false;
  constants = [];
