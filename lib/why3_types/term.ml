...
include Why3.Term
  ...
  let lsymbol : 'c 'csts . (lsymbol, 'c, 'csts) Elpi.API.ContextualConversion.t =
    let pp_doc fmt () =
      Format.fprintf fmt "%% Embedding of predicate symbols. Name, argument and value type can be@\n";
  ...
  [@@elpi.opaque {
    ...
    pp = (pp_why_data (fun fmt c -> match c with
      | Why3.Constant.ConstInt n -> Format.fprintf fmt "%s" (Why3.BigInt.to_string n.il_int)
...
type ctx_for_term =
| Ctx_vs of (WTerm.vsymbol[@elpi.key])
(* Now the PPX can generate this
  [@elpi.code "ctx-vs" "term -> var -> prop"] *)
[@@elpi.index (module Vsym_tags) "term"]
[@@deriving elpi {declaration}]
[@@elpi.pp fun fmt v -> match v with Ctx_vs v -> Format.fprintf fmt "%a" WPretty.print_vs v]
let pp_ctx_for_term = fun fmt c -> ctx_for_term.pp fmt (0,c)

type ctx_for_lsymbol =
| Ctx_ls of (WTerm.lsymbol[@elpi.key])
[@@elpi.index (module Lsym_tags) "term"]
[@@deriving elpi {declaration}]
[@@elpi.pp fun fmt v -> match v with Ctx_ls ls -> Format.fprintf fmt "%a" WPretty.print_ls ls]
...
[@@elpi.type_doc "Pattern constructors for pattern-matching terms. Use pvar to name pattern-bound variables and babs to bind their body occurrences."]
[@@elpi.pp fun fmt _ -> Format.fprintf fmt "<pattern>"]
and why_simple_branch =
  | Branch of why_simple_pattern * why_simple_term
  | Babs of
      WTerm.vsymbol *
      (why_simple_branch [@elpi.binder "term" ctx_for_term (fun v -> Ctx_vs v)])
[@@elpi.type_code "branch"]
[@@elpi.type_doc "Case branches. Use babs to bind pattern variables, then branch to pair the pattern with its body."]
...
and why_simple_term =
  ...
  | Tlet of
      why_simple_term *
      WTerm.vsymbol *
      (why_simple_term [@elpi.binder "term" ctx_for_term (fun _t v -> Ctx_vs v)])
    | Ttrigger of why_simple_term list list * why_simple_term
  | Tquant of
      WTerm.quant *
      WTerm.vsymbol *
      (why_simple_term [@elpi.binder "term" ctx_for_term (fun _q v -> Ctx_vs v)])
  | Teps of
      WTerm.vsymbol *
...
[@@elpi.type_code "term"]
[@@elpi.type_doc "Why3 terms. Use tattr to attach attributes to a subterm. Pattern-matching uses branch/babs HOAS branches, applications have optional type annotations, and quantifier triggers use an explicit ttrigger carrier directly under the quantifier body."]
...
let pp_attrs fmt attrs =
  ...
  | _ ->
      Format.fprintf fmt "[@%a] "
        (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ") pp_attribute)
        attrs
...
let rec pp_simple_pattern fmt p =
  ...
  | Papp (ls, args) ->
      Format.fprintf fmt "%a(%a)" WPretty.print_ls ls
        (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ") pp_simple_pattern)
        args
  | Por (p1, p2) -> Format.fprintf fmt "%a | %a" pp_simple_pattern p1 pp_simple_pattern p2
  | Pas (p, v) -> Format.fprintf fmt "%a as %a" pp_simple_pattern p WPretty.print_vs v
and pp_simple_branch fmt branch =
  match branch with
  | Branch (pattern, body) -> Format.fprintf fmt "%a => %a" pp_simple_pattern pattern pp_simple_term body
  | Babs (v, branch) -> Format.fprintf fmt "(%a => %a)" WPretty.print_vs v pp_simple_branch branch
and pp_simple_term =
  ...
  | Tapp (ls, args, _ty) ->
    Format.fprintf fmt "%a(%a)" WPretty.print_ls ls (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ") pp_simple_term) args
  | Tlet (t1, v, t2) ->
    Format.fprintf fmt "let %a = %a in %a" WPretty.print_vs v pp_simple_term t1 pp_simple_term t2
  | Ttrigger (tr, t) ->
    let pp_one fmt terms =
      Format.fprintf fmt "[%a]"
        (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ") pp_simple_term)
        terms
    in
    Format.fprintf fmt "trigger {%a} in %a"
      (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ") pp_one)
      tr pp_simple_term t
  | Tquant (q, v, t) ->
    Format.fprintf fmt "%a %a. %a" (WPretty.print_quant) q WPretty.print_vs v pp_simple_term t
  | Teps (v, t) ->
  ...
  | Tbinop (op, t1, t2) ->
    Format.fprintf fmt "(%a %a %a)" pp_simple_term t1 (WPretty.print_binop ~asym:false) op pp_simple_term t2
  | Tif (t1, t2, t3) ->
    Format.fprintf fmt "if %a then %a else %a" pp_simple_term t1 pp_simple_term t2 pp_simple_term t3
  | Tnot t ->
    Format.fprintf fmt "not %a" pp_simple_term t
  | Tcase (t, branches) ->
    Format.fprintf fmt "case %a of %a" pp_simple_term t
      (Format.pp_print_list ~pp_sep:Why3.Pp.comma pp_simple_branch) branches

let rec pp_decl_body fmt = function
  | Dterm t -> pp_simple_term fmt t
  | Dabs (v, body) -> Format.fprintf fmt "%a => %a" WPretty.print_vs v pp_decl_body body
...
let wrap_term_attrs t st =
  wrap_attrs (term_attrs t) st
...
let rec simple_pattern_bound_vars_in_order (p : why_simple_pattern) : WTerm.vsymbol list =
  match p with
  | Pwild -> []
  | Pvar v -> [v]
  ...
  | Por (p1, p2) ->
      ...
      else pattern_type_error "or-pattern branches must bind the same variables in the same order"
  | Pas (p, v) -> simple_pattern_bound_vars_in_order p @ [v]

let rec simple_pattern_to_pattern expected_ty (p : why_simple_pattern) : WTerm.pattern =
  match p with
  ...
  | Papp (ls, args) ->
      ...
      in
      let subst =
        try WTy.ty_match WTy.Mtv.empty result_ty expected_ty with _ ->
          pattern_type_error
            "pattern constructor %a returns %a but is used at type %a"
            WPretty.print_ls ls WPretty.print_ty result_ty WPretty.print_ty expected_ty
      in
...
let rec term_to_simple_term (tm : WTerm.term) : why_simple_term =
  ...
  | Tapp (ls, args) ->
    let ty_opt =
      ...
      | None -> None
      | Some ty ->
        (* let has_free_tvs = not (Why3.Ty.Stv.is_empty (Why3.Ty.ty_freevars Why3.Ty.Stv.empty ty)) in
        if has_free_tvs then None else *)
          Some (ty_to_why_simple_ty ty)
    in
    wrap_term_attrs tm (Tapp (ls, List.map term_to_simple_term args, ty_opt))
  | Tif (t1, t2, t3) ->
    wrap_term_attrs tm (Tif (term_to_simple_term t1, term_to_simple_term t2, term_to_simple_term t3))
  | Tlet (t1, tb) ->
    let (v, t2) = WTerm.t_open_bound tb in
    wrap_term_attrs tm (Tlet (term_to_simple_term t1, v, term_to_simple_term t2))
  ...
  | Tbinop (op, t1, t2) ->
    wrap_term_attrs tm (Tbinop (op, term_to_simple_term t1, term_to_simple_term t2))
...
and term_branch_to_simple_term (branch : WTerm.term_branch) : why_simple_branch =
  ...
  let branch = Branch (pattern_to_simple_pattern pattern, term_to_simple_term term) in
  List.fold_right (fun v acc -> Babs (v, acc)) (pattern_bound_vars_in_order pattern) branch
...
let rec simple_term_to_term (st : why_simple_term) : WTerm.term =
  ...
  | Tif (t1, t2, t3) ->
    WTerm.t_if (simple_term_to_term t1) (simple_term_to_term t2) (simple_term_to_term t3)
  ...
  | Tcase (t, branches) ->
    ...
    let simple_branch_to_term_branch branch =
      ...
      if not (same_vsymbol_list binders pattern_vars) then
        pattern_type_error
          "branch binders must match the pattern variables in left-to-right order";
...
let simple_term : 'c 'csts . (why_simple_term, 'c, 'csts) Elpi.API.ContextualConversion.t =
...
let rec focused_goal_to_tdecls (goal : focused_goal) : Why3.Theory.tdecl list =
  ...
  | Goal_formula (pr, concl) ->
    [ Why3.Theory.create_decl
        (Why3.Decl.create_prop_decl Why3.Decl.Pgoal pr (simple_term_to_term concl)) ]
  | Local_symbol (ls, decl, body) ->
      ...
      in
      Why3.Theory.create_decl local_decl
