...
let prsymbol : (prsymbol, 'a, 'b) Elpi.API.ContextualConversion.t =
  Term_conv.prsymbol
...
[@@elpi.type_doc
  "Classification of Why3 declarations returned by why3.decl-kind."]
[@@elpi.pp
  fun fmt -> function
...
[@@elpi.opaque
  ...
  ; doc =
      "Opaque Why3 declaration. Inspect it with why3.decl-kind, \
       why3.decl-defines, and why3.decl-body."
...
let logic_decl_lsymbol ((ls, _) : logic_decl) = ls
...
let decl_defined_grefs (decl : Decl.decl) =
  ...
  | Decl.Dlogic ldecls ->
    List.map (fun ld -> Gls (logic_decl_lsymbol ld)) ldecls
