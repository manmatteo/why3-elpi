...
let split_goal_tdecls tdecls =
  let rec aux acc = function
    ...
    | td :: rest -> (
      ...
      | Why3.Theory.Decl d -> (
        match Term.goal_decl_to_focused_goal d with
...
let embed_task : (task, 'a, 'b) Elpi.API.ContextualConversion.embedding =
 fun ~depth hyp constraints state task ->
  (Elpi_api_compat.BuiltInContextualData.list tdecl).embed ~depth hyp
    constraints state (task_tdecls task)
...
let readback_task : (task, 'a, 'b) Elpi.API.ContextualConversion.readback =
  ...
  fun ~depth hyp c st term ->
    let st, tdecl_list, eg =
      (Elpi_api_compat.BuiltInContextualData.list tdecl).readback ~depth hyp c
        st term
    in
...
let focused_task_c =
  Elpi.API.RawData.Constants.declare_global_symbol "focused-task"
...
let embed_focused_task : (task, 'a, 'b) Elpi.API.ContextualConversion.embedding
 ...
 fun ~depth hyp constraints state task ->
  ...
  | None ->
    Elpi.API.Utils.error "focused-task embedding requires a task with a goal"
  | Some (rest, goal) ->
    let state, rest_tm, eg1 =
      (Elpi_api_compat.BuiltInContextualData.list tdecl).embed ~depth hyp
        constraints state rest
    in
    let state, goal_tm, eg2 =
      Term.focused_goal.embed ~depth hyp constraints state goal
    in
    ( state
    , Elpi.API.RawData.mkAppGlobalL focused_task_c [ rest_tm; goal_tm ]
    , eg1 @ eg2 )

let readback_focused_task :
    (task, 'a, 'b) Elpi.API.ContextualConversion.readback =
 fun ~depth hyp constraints state tm ->
  ...
  | Elpi.API.RawData.App (hd, rest_tm, [ goal_tm ]) when hd == focused_task_c ->
    let state, rest, eg1 =
      (Elpi_api_compat.BuiltInContextualData.list tdecl).readback ~depth hyp
        constraints state rest_tm
    in
    let state, goal, eg2 =
      Term.focused_goal.readback ~depth hyp constraints state goal_tm
    in
...
let focused_task : (task, 'a, 'b) Elpi.API.ContextualConversion.t =
  ...
  ; pp_doc =
      (fun fmt () ->
        ...
        Format.fprintf fmt
          "external symbol focused-task : list tdecl -> focused-goal -> \
           focused-task.@\n")
...
let elpi_constant_type_envc =
  Elpi.API.RawData.Constants.declare_global_symbol elpi_constant_type_env
...
let env : 'c. (env, 'c, 'csts) Elpi.API.ContextualConversion.t =
  let { Elpi.API.Conversion.embed; readback; ty; pp_doc; pp } =
    elpi_opaque_data_decl_env
  in
...
class ctx_for_env (h : Elpi.API.Data.hyps) (_s : Elpi.API.Data.state) :
  Ctx_for_env.t =
  object (_)
