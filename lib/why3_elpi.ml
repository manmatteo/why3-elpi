...
let _debug_no_typecheck = Why3.Debug.register_flag ~desc:"Disable typechecking for Elpi transformations" "no_elpi_tc"

let cached_program : (string * API.Setup.elpi * API.Compile.program) option ref = ref None
...
let elpi_builtins () =
  let builtins =
    declaration @ why3_builtin_declarations @ List.rev !extra_builtin_declarations
  in
  document builtins;
  ...
  try
    let _st, tm, _eg =
      (Elpi_api_compat.BuiltInContextualData.list conv).readback
        ~depth:0 [] API.RawData.no_constraints state output_term
    in
...
let declare_external_symbol ~name ~ty =
  register_builtin_declaration
    (API.BuiltIn.LPCode
       (Printf.sprintf "external symbol %s : %s." name ty));
