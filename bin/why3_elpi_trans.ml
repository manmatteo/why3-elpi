...
let transform_query ~file build_query (t : Why3.Task.task) =
  ...
  | Some (rest, goal) -> (
    ...
    match
      Why3_elpi.run_query_with prog
        (fun state ->
          let depth = 0 in
          let state, rest_t, eg1 =
            (Elpi_api_compat.BuiltInContextualData.list tdecl).embed ~depth []
              Elpi.API.RawData.no_constraints state rest
          in
          let state, goal_t, eg2 =
            focused_goal.embed ~depth [] Elpi.API.RawData.no_constraints state
              goal
          in
          ...
          in
          let output_t =
            Elpi.API.RawData.mkUnifVar output_uvar ~args:[] state
          in
